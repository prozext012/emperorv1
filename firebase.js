// ===== FIREBASE (Firestore) — buat notifikasi & testimoni realtime =====
    ...
    // Sebagian browser cuma izinin audio otomatis kalau user udah pernah sentuh halamannya —
    // ini buat "mencairkan" audio begitu ada sentuhan pertama, biar sound berikutnya pasti bunyi.
    document.addEventListener('click', () => { const c = getNotifAudioCtx(); if (c && c.state === 'suspended') c.resume().catch(() => {}); }, { once: true });
    document.addEventListener('touchstart', () => { const c = getNotifAudioCtx(); if (c && c.state === 'suspended') c.resume().catch(() => {}); }, { once: true });
