// ===== FIREBASE (Firestore) — buat notifikasi & testimoni realtime =====
    ...
    function tryPlayNotifSound(pesan) {
        ...
        if (ctx && ctx.state === 'suspended') {
            ctx.resume().then(() => {
                if (ctx.state === 'running') { playNotifSound().then(() => speakNotifText(pesan)); }
                else { pendingNotifSpeech = pesan; }
            }).catch(() => { pendingNotifSpeech = pesan; });
            // langsung coba juga, siapa tau browsernya gak strict
            if (ctx.state === 'running') playNotifSound().then(() => speakNotifText(pesan));
        } else {
            playNotifSound().then(() => speakNotifText(pesan));
        }
    }
    ...
    ['click', 'touchstart', 'keydown'].forEach(evt => {
        document.addEventListener(evt, retryPendingNotifSound, { once: true });
    });
    ...
    function showNotifOverlayNow() {
        ...
        try { if (sessionStorage.getItem('notifOverlaySeen')) { notifOverlayDone = true; return; } } catch (e) {}
        if (!window.__notifData || window.__notifData.length === 0) return; // data belum siap, coba lagi nanti
        const el = document.getElementById('notifOverlayPreview');
        if (!el) return;
