// ===== HELPER: FORMAT RUPIAH =====
    ...
    function makeDesc(sections) {
        return sections.map(s => {
            ...
            if (s.type === 'image') return `<img src="${s.src}" alt="Foto produk" style="width:100%;border-radius:12px;margin:8px 0;display:block;" />`;
            return '';
        }).join('');
    }
...
(function () {
    ...
    function markSegsBefore(i) {
        ...
        segs.forEach((el, idx) => {
            if (idx < i) { el.classList.remove('run'); el.classList.add('filled'); }
            else if (idx > i) { el.classList.remove('run', 'filled'); el.style.width = '0%'; }
        });
    }

    function clearMedia() {
        statusMediaWrap.querySelectorAll('.status-media-img, .status-media-video, .status-media-text, .status-media-caption').forEach(el => el.remove());
    }
    ...
    function resumeProgress() {
        ...
        advanceTimer = setTimeout(goNext, remaining);
            ...
            span.addEventListener('click', function () {
                ...
                if (window.__sendPesanMasuk) {
                    window.__sendPesanMasuk({ type: 'emoji', text: emoji, statusId: current ? current.id : null })
                        .then(() => showToast('Reaksi terkirim'))
                        .catch(() => showToast('Gagal mengirim reaksi'));
                }
            });
        });
    }
    ...
    function handlePickedImage(file) {
        if (!file) return;
        pauseProgress();
        fileToCompressedBase64(file, 900, 0.7).then(base64 => {
            if (base64.length > 900000) {
                showToast('Ukuran foto terlalu besar, coba foto lain');
                resumeProgress();
                return;
            }
            const list = window.__statusData || [];
            ...
            if (window.__sendPesanMasuk) {
                window.__sendPesanMasuk({ type: 'image', img: base64, statusId: current ? current.id : null })
                    .then(() => showToast('Foto terkirim'))
                    .catch(() => showToast('Gagal mengirim foto'));
            }
            resumeProgress();
        }).catch(() => { showToast('Gagal memproses foto'); resumeProgress(); });
    }
            ...
            reader.onload = () => {
                ...
                if (window.__sendPesanMasuk) {
                    window.__sendPesanMasuk({ type: 'voice', audio: reader.result, statusId: current ? current.id : null })
                        .then(() => showToast('Pesan suara terkirim'))
                        .catch(() => showToast('Gagal mengirim pesan suara'));
                }
                resetRecordingUI();
            };
